...
const apiRequest = async (url: string, options = {}) => {
  ...
  const fetchFunciones = async () => {
    try {
      setLoading(true)
      const response = await apiRequest('/funciones-sustantivas') // Quita process.env.NEXT_PUBLIC_API_URL
      
      if (!response.ok) {
        throw new Error("Error al cargar funciones sustantivas")
      }
    ...
    try {
      ...
      if (editingId) {
        ...
        const response = await apiRequest(
          `/funciones-sustantivas/${editingId}`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "PUT",
        ...
        toast({
          title: "Éxito",
          description: "Función sustantiva actualizada correctamente",
        })
      } else {
        // Crear nueva función - CORREGIDO
        const response = await apiRequest(
          `/funciones-sustantivas`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "POST",
            body: JSON.stringify(formData),
          }
        ...
        toast({
          title: "Éxito",
          description: "Función sustantiva creada correctamente",
        })
      }
      ...
      toast({
        title: "Error",
        description: error.message || "Error al guardar la función sustantiva",
        variant: "destructive",
      })
    } finally {
      setSubmitting(false)
...
const handleEdit = (funcion: FuncionSustantiva) => {
  setFormData({
  ...
  const handleToggleStatus = async (funcion: FuncionSustantiva) => {
    try {
      setLoading(true)
      ...
      // CORREGIDO
      const response = await apiRequest(
        `/funciones-sustantivas/${funcion.id}/estado`, // Quitar process.env.NEXT_PUBLIC_API_URL
        {
          method: "PATCH",
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-emerald-700 text-white px-6 py-4 rounded-t-lg">
            <h1 className="text-2xl font-bold">FUNCIONES SUSTANTIVAS</h1>
          </div>
          ...
          <div className="bg-white rounded-b-lg shadow-lg p-6">
              ...
              <CardContent className="p-6">
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      id="nombre"
                      placeholder="Ingrese la función sustantiva"
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg font-semibold">Funciones Sustantivas Registradas</CardTitle>
              </CardHeader>
              <CardContent>
                      ...
                      <TableBody>
                        {funciones.map((funcion, index) => (
                          <TableRow key={funcion.id} className="hover:bg-gray-50">
                            ...
                            <TableCell>{funcion.descripcion || "-"}</TableCell>
                            <TableCell>
                              <Badge
                        ...
                        {funciones.length === 0 && !loading && (
                          <TableRow>
                            <TableCell colSpan={6} className="text-center py-8 text-gray-500">
                              No hay funciones sustantivas registradas
                            </TableCell>
                          </TableRow>
                        )}
                      </TableBody>

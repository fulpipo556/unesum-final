...
export default function FuncionesSustantivasPage() {
  ...
  const [funciones, setFunciones] = useState<Organizacion[]>([])
...
const apiRequest = async (url: string, options = {}) => {
  ...
  const fetchFunciones = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error("Error al cargar las Unidades de organización curricular")
      }
      
      const data = await response.json()
      setFunciones(data.data || [])
    } catch (error) {
      console.error("Error al cargar las Unidades de organización curricular:", error)
      toast({
        title: "Error",
        description: "No se pudieron cargar las Unidades de organización curricular",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    try {
      ...
      if (editingId) {
        ...
        const response = await apiRequest(
          `/organizacion_curricular/${editingId}`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "PUT",
        ...
        if (!response.ok) {
          ...
          throw new Error(error.message || "Error al actualizar las Unidades de organización curricular")
        }
        
        toast({
          title: "Éxito",
          description: "Unidad de organizacion curricular actualizado correctamente",
        })
      } else {
        // Crear nueva función - CORREGIDO
        const response = await apiRequest(
          `/organizacion_curricular`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "POST",
            body: JSON.stringify(formData),
          }
        ...
        if (!response.ok) {
          const error = await response.json()
          throw new Error(error.message || "Error al crear la Unidad de Organización Curricular")
        }
        
        toast({
          title: "Éxito",
          description: "Unidad de organización curricular creado correctamente",
        })
      }
  ...
  const handleDelete = async (id: string) => {
    if (confirm("¿Estás seguro de que deseas eliminar esta Unidad de organización curricular?")) {
      try {
        setLoading(true)
        // CORREGIDO
        const response = await apiRequest(
          `/organizacion_curricular/${id}`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "DELETE",
          }
        )
        ...
        if (!response.ok) {
          throw new Error("Error al eliminar la Unidad de organización curricular")
        }
        
        toast({
          title: "Éxito",
          description: "Unidad de organización curricular eliminado correctamente",
        })
        ...
        toast({
          title: "Error",
          description: "No se pudo eliminar la Unidad de organización curricular",
          variant: "destructive",
        })
      } finally {
        setLoading(false)
...
const handleEdit = (funcion: Organizacion) => {
  ...
  const handleToggleStatus = async (funcion: Organizacion) => {
    try {
      setLoading(true)
      ...
      // CORREGIDO
      const response = await apiRequest(
        `/organizacion_curricular/${funcion.id}/estado`, // Quitar process.env.NEXT_PUBLIC_API_URL
        {
          method: "PATCH",
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-emerald-700 text-white px-6 py-4 rounded-t-lg">
            <h1 className="text-2xl font-bold">Unidad de Organización Curricular</h1>
          </div>
          ...
          <div className="bg-white rounded-b-lg shadow-lg p-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg font-semibold">Unidad de Organización Curricular Registrada</CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-gray-50">
                          ...
                          <TableHead className="font-semibold">Unidad de Organización Curricular</TableHead>
                        ...
                        {funciones.length === 0 && !loading && (
                          <TableRow>
                            <TableCell colSpan={6} className="text-center py-8 text-gray-500">
                              No hay unidad de organización curricular registrados
                            </TableCell>
                          </TableRow>
                        )}
                      </TableBody>

...
const apiRequest = async (url: string, options = {}) => {
  ...
  const fetchFunciones = async () => {
    try {
      setLoading(true)
      const response = await apiRequest('/periodo') 
      
      if (!response.ok) {
        throw new Error("Error al cargar los periodo")
      }
      
      const data = await response.json()
      setFunciones(data.data || [])
    } catch (error) {
      console.error("Error al cargar los periodo:", error)
      toast({
        title: "Error",
        description: "No se pudieron cargar los periodo",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    try {
      ...
      if (editingId) {
        ...
        if (!response.ok) {
          ...
          throw new Error(error.message || "Error al actualizar los periodos")
        }
        ...
        if (!response.ok) {
          const error = await response.json()
          throw new Error(error.message || "Error al crear el periodo")
        }
  ...
  const handleDelete = async (id: string) => {
    if (confirm("¿Estás seguro de que deseas eliminar este periodo?")) {
      try {
        ...
        // CORREGIDO
        const response = await apiRequest(
          `/periodo/${id}`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "DELETE",
          }
        )
...
const handleEdit = (funcion: Periodo) => {
  ...
  const handleToggleStatus = async (funcion: Periodo) => {
    try {
      setLoading(true)
      ...
      // CORREGIDO
      const response = await apiRequest(
        `/periodo/${funcion.id}/estado`, // Quitar process.env.NEXT_PUBLIC_API_URL
        {
          method: "PATCH",
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-emerald-700 text-white px-6 py-4 rounded-t-lg">
            <h1 className="text-2xl font-bold">Periodo</h1>
          </div>

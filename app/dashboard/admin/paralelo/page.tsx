...
export default function FuncionesSustantivasPage() {
  ...
  const [funciones, setFunciones] = useState<Paralelo[]>([])
...
const apiRequest = async (url: string, options = {}) => {
  ...
  const fetchFunciones = async () => {
    try {
      setLoading(true)
      const response = await apiRequest('/paralelos') // Quita process.env.NEXT_PUBLIC_API_URL
      ...
      console.error("Error al cargar los paralelos:", error)
      toast({
        title: "Error",
        description: "No se pudieron cargar los paralelos",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    try {
      ...
      if (editingId) {
        ...
        const response = await apiRequest(
          `/paralelos/${editingId}`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "PUT",
        ...
        if (!response.ok) {
          ...
          throw new Error(error.message || "Error al actualizar los pararlelos")
        }
        ...
        const response = await apiRequest(
          `/paralelos`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "POST",
            body: JSON.stringify(formData),
          }
        ...
        if (!response.ok) {
          const error = await response.json()
          throw new Error(error.message || "Error al crear el paralelo")
        }
        
        toast({
          title: "Éxito",
          description: "Paralelo creado correctamente",
        })
      }
...
const handleEdit = (funcion: Paralelo) => {
  ...
  const handleToggleStatus = async (funcion: Paralelo) => {
    try {
      setLoading(true)
      ...
      // CORREGIDO
      const response = await apiRequest(
        `/paralelos/${funcion.id}/estado`, // Quitar process.env.NEXT_PUBLIC_API_URL
        {
          method: "PATCH",
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-emerald-700 text-white px-6 py-4 rounded-t-lg">
            <h1 className="text-2xl font-bold">PARALELOS</h1>
          </div>
          ...
          <div className="bg-white rounded-b-lg shadow-lg p-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg font-semibold">Paralelo Registrado</CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-gray-50">
                          ...
                          <TableHead className="font-semibold">Paralelos</TableHead>

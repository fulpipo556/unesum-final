...
export default function FuncionesSustantivasPage() {
  ...
  const [funciones, setFunciones] = useState<Rol[]>([])
...
const apiRequest = async (url: string, options = {}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nombre) {
      toast({
        title: "Error",
        description: "El rol es un campo obligatorio",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      if (editingId) {
        ...
        const response = await apiRequest(
          `/roles/${editingId}`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "PUT",
        ...
        const response = await apiRequest(
          `/roles`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "POST",
            body: JSON.stringify(dataToSend),
          }
  ...
  const handleDelete = async (id: string) => {
    if (confirm("¿Estás seguro de que deseas eliminar este rol?")) {
      try {
        ...
        // CORREGIDO
        const response = await apiRequest(
          `/roles/${id}`, // Quitar process.env.NEXT_PUBLIC_API_URL
          {
            method: "DELETE",
          }
        )
...
const handleEdit = (funcion: Rol) => {
  ...
  const handleToggleStatus = async (funcion: Rol) => {
    try {
      setLoading(true)
      ...
      // CORREGIDO
      const response = await apiRequest(
        `/roles/${funcion.id}/status`, // Endpoint correcto
        {
          method: "PATCH",
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-white rounded-b-lg shadow-lg p-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg font-semibold">Rol Registrado</CardTitle>
              </CardHeader>
              <CardContent>
